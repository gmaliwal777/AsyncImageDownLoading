...
-(void)downLoadingAsynImage:(UIImageView *)imgViewReference imagePath:(NSString *)imagePath  backgroundQueue:(dispatch_queue_t)queue imageObj:(UIImage *__autoreleasing*)imageObj{
...
-(void)showImage{
    if (_imgView) {
        ...
        if (imageRef) {
            imageRef = _imgView.image;
        }
        
        
        UIView *contentView = [_imgView superview];
        UIActivityIndicatorView *activity = (UIActivityIndicatorView *)[contentView viewWithTag:1];
...
-(void)dealloc{
    NSLog(@"async downloading dealloc");
    self.connection         = nil;
    self.imgData            = nil;
